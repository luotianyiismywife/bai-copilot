...
async function ensureTokenizerFile() {
	...
	if (fs.existsSync(destFile)) {
		console.log("[bai-copilot] Tokenizer file already exists at:", destFile);
		return;
	}

	console.log("[bai-copilot] Downloading tokenizer file from:", TOKENIZER_URL);
	try {
		await downloadFile(TOKENIZER_URL, destFile);
		console.log("[bai-copilot] Tokenizer file downloaded to:", destFile);
	} catch (err) {
		console.warn("[bai-copilot] Failed to download tokenizer file:", err.message);
		console.warn("[bai-copilot] Token counting will fall back to estimation.");
	}
}

ensureTokenizerFile().catch((err) => {
	console.warn("[bai-copilot] Tokenizer setup failed:", err.message);
	console.warn("[bai-copilot] Token counting will fall back to estimation.");
});
