 ...
 * Startup model sync.
 *
 * On every VS Code window open the extension checks whether the Bai
 ...
 * single log line in the extension's Output channel ("Bai") — no
...
const SYNC_DATE_KEY = "bai.lastModelSyncDate";
...
export async function syncModelsOnStartup(context: vscode.ExtensionContext): Promise<void> {
    try {
        const config = vscode.workspace.getConfiguration("bai");
