...
import { getAnthropicSupportedModelIds } from "../apiModelList";
...
async function performCommitMsgGeneration(secrets: vscode.SecretStorage, gitDiff: string, inputBox: any, repoPath?: string) {
    ...
    try {
        vscode.commands.executeCommand("setContext", "bai.isGeneratingCommit", true);
        const config = vscode.workspace.getConfiguration();

        const customSystemPrompt = config.get<string>("bai.commitMessagePrompt", "");
        let systemPrompt = customSystemPrompt || DEFAULT_PROMPT.system;

        // Fetch recent commits for style reference
        const recentCommitsCount = config.get<number>("bai.recentCommitsCount", 10);
        const includeCommitDiff = config.get<boolean>("bai.commitIncludeCommitDiff", false);
        ...
        const attachContextFiles = config.get<boolean>("bai.commitAttachContextFiles", true);
        ...
        const commitModelId = config.get<string>("bai.commitModel", "deepseek-v4-flash");
        // Fetch full model config (apiMode, max_completion_tokens, extra, etc.)
        const selectedModel: BaiModelItem = getBuiltInModelConfig(commitModelId) ?? { id: commitModelId, owned_by: "bai" };
        ...
        const baseUrl = selectedModel.baseUrl || "https://api.b.ai/v1/";
        ...
        const commitLanguage = config.get<string>("bai.commitLanguage", "auto");
        ...
        // (supported_endpoint_types) — no model IDs are hardcoded.
        ...
        } else if (selectedModel.apiMode === "anthropic") {
            apiMode = "anthropic";
        } else {
            const anthropicModelIds = await getAnthropicSupportedModelIds(primaryEntry.value);
            if (enableAnthropicApi && anthropicModelIds.has(commitModelId)) {
                apiMode = "anthropic";
            } else {
                apiMode = "openai";
            }
        ...
        // the correct headers (x-api-key for anthropic, Bearer for openai).
        ...
        while (true) {
            ...
            // Balance ≤ minBalancePoints (default 0) → skip this key and try the next.
        ...
        vscode.commands.executeCommand("setContext", "bai.isGeneratingCommit", false);
    }
}
