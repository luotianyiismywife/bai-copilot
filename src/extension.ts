...
import { BaiChatModelProvider } from "./provider";
...
const WELCOME_SHOWN_KEY = "bai.welcomeShown";

/** Walkthrough contribution ID (publisher.extension#walkthroughId). */
const WALKTHROUGH_ID = "luotianyiismywife.bai-copilot-provider#baiGettingStarted";

/**
 * 格式化积分余额详情为显示文本：总积分 + 赠送（将过期）积分。
 * B.AI 余额分「总积分」与「赠送/限时积分」两部分，分开显示；
 * 总积分 ≤ minBalancePoints 时以 error 图标标记（轮询会被跳过）。
 */
function formatBalanceDetailText(detail: BalanceDetail | undefined, minBalance: number): string {
    if (!detail) {
        return "";
    }
    const total = detail.pointsBalance;
    const expiring = detail.pointsExpiring;
    const totalIcon = total > minBalance ? "$(coin)" : "$(error)";
    const parts: string[] = [`${totalIcon} ${l10n("Balance")} ${formatPoints(total)}`];
    if (expiring > 0) {
        parts.push(`$(gift) ${l10n("Gift")} ${formatPoints(expiring)}`);
    }
    return parts.join(" · ");
}

/** 积分格式化：千分位分隔（如 300,000） */
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const provider = new BaiChatModelProvider(context.secrets, tokenCountStatusBarItem);

    // Register the Bai provider under the vendor id used in package.json
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration("bai.apiMode") || e.affectsConfiguration("bai.enableAutoModelDiscovery")) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.setApiKey", async () => {
            ...
            const apiKey = await vscode.window.showInputBox({
                title: l10n("Bai Provider API Key"),
                prompt: existing ? l10n("Update your Bai API key") : l10n("Enter your Bai API key"),
                ignoreFocusOut: true,
                password: true,
                value: existing ?? "",
            });
            ...
            if (!apiKey.trim()) {
                // Clear all keys
                await context.secrets.store("bai.apiKeys", JSON.stringify({ keys: [], activeIndex: 0 }));
                await context.secrets.delete("bai.apiKey");
                vscode.window.showInformationMessage(l10n("Bai API key cleared."));
                return;
            }
            const trimmed = apiKey.trim();
            if (existing && existing === trimmed) {
                vscode.window.showInformationMessage(l10n("Bai API key saved."));
                return;
            }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.manageApiKeys", async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.setVisionProxyModel", async () => {
            const config = vscode.workspace.getConfiguration();
            const current = config.get<string>("bai.visionProxyModel", "kimi-k2.6");
            ...
            await config.update("bai.visionProxyModel", newModel, vscode.ConfigurationTarget.Global);
    ...
    // Command to open the Bai website to get an API key
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.getApiKey", () => {
            vscode.env.openExternal(vscode.Uri.parse("https://chat.b.ai/chat"));
        })
    );

    // Command to open extension settings
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.openSettings", () => {
            vscode.commands.executeCommand("workbench.action.openSettings", "@ext:luotianyiismywife.bai-copilot-provider");
        })
    );

    // Register the generateGitCommitMessage command handler
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.generateGitCommitMessage", async (scm) => {
            generateCommitMsg(context.secrets, scm);
        }),
        vscode.commands.registerCommand("bai.abortGitCommitMessage", () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("bai.setModelPreset", async () => {
            const config = vscode.workspace.getConfiguration();
            const presets = config.get<ModelPreset[]>("bai.modelPresets", []);
            const currentPresetId = config.get<string>("bai.modelPreset", "custom");
            const currentTemp = config.get<number | null>("bai.temperature", null);
            const currentTopP = config.get<number | null>("bai.top_p", null);
            ...
            if (presetId) {
                ...
                if (matchedPreset) {
                    await config.update("bai.modelPreset", matchedPreset.id, vscode.ConfigurationTarget.Global);
                    await config.update("bai.temperature", matchedPreset.temperature, vscode.ConfigurationTarget.Global);
                    vscode.window.showInformationMessage(
                ...
                if (inputValue !== undefined) {
                    ...
                    await config.update("bai.modelPreset", "custom", vscode.ConfigurationTarget.Global);
                    await config.update("bai.temperature", tempNum, vscode.ConfigurationTarget.Global);
                    if (parts.length === 2) {
                        const topPNum = parseFloat(parts[1].trim());
                        await config.update("bai.top_p", topPNum, vscode.ConfigurationTarget.Global);
                        vscode.window.showInformationMessage(
    ...
    // Startup model sync — checks for new Bai models at most once per
    // day and logs a single line to the "Bai" Output channel.
...
async function showApiKeyManager(context: vscode.ExtensionContext): Promise<void> {
        ...
        if (store.keys.length === 0) {
            ...
            store.keys.forEach((entry, i) => {
                ...
                const balanceText = detail
                    ? formatBalanceDetailText(detail, getMinBalancePoints())
                    : entry.cookie
    ...
    const addKeyFlow = async (): Promise<boolean> => {
        const keyValue = await vscode.window.showInputBox({
            ...
            prompt: l10n("Enter your Bai API key"),
            ignoreFocusOut: true,
            password: true,
        });
                ...
                if (result.ok === true) {
                    ...
                    if (result.reason === "balance") {
                        vscode.window.showWarningMessage(
                            l10nFormat("Key balance is insufficient (≤ {0} points)", String(getMinBalancePoints()))
                        );
                    } else {
    ...
    const bindCookieFlow = async (index: number): Promise<void> => {
        ...
        const cookie = await vscode.window.showInputBox({
            ...
            prompt: l10n("Enter the __Secure-authjs.session-token cookie value for this key"),
            ignoreFocusOut: true,
            password: true,
            value: entry.cookie ?? "",
        });
    ...
    const editKeyFlow = async (index: number): Promise<void> => {
        ...
        const newCookie = await vscode.window.showInputBox({
            title: l10n("Edit API Key"),
            prompt: l10n("Edit the __Secure-authjs.session-token cookie (empty to clear)"),
            ignoreFocusOut: true,
            password: true,
            value: entry.cookie ?? "",
        });
