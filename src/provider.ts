...
import type { ModelPreset, BaiModelItem } from "./types";

import { createRetryConfig, executeWithRetry, convertToolsToOpenAI } from "./utils";

import { prepareLanguageModelChatInformation, getAutoDiscoveredModelConfig, getAnthropicModelIds } from "./provideModel";
 ...
 * controlled via the "bai.enableThirdPartyTokenIndicator" setting.
 */
...
/**
 * VS Code Chat provider backed by Bai API.
 */
export class BaiChatModelProvider implements LanguageModelChatProvider {
     ...
     * Lets VS Code re-query the model list when `bai.apiMode` changes,
    ...
    async provideLanguageModelChatResponse(
        ...
        const trackingProgress: Progress<LanguageModelResponsePart> = {
            report: (part) => {
                try {
                    ...
                    console.error("[Bai] Progress.report failed", {
                        modelId: model.id,
        ...
        try {
            ...
            let um: BaiModelItem | undefined = getBuiltInModelConfig(model.id);
            if (!um) {
            ...
            if (um) {
                if (um.supportsTemperature !== false) {
                    const tempPreset = config.get<string>("bai.modelPreset", "custom");
                    if (tempPreset !== "custom") {
                        ...
                        const userTemperature = config.get<number | null>("bai.temperature", null);
                        if (userTemperature !== null) {
                            um.temperature = userTemperature;
                        }
                        const userTopP = config.get<number | null>("bai.top_p", null);
                        if (userTopP !== null) {
                            um.top_p = userTopP;
                        } else {
            ...
            // bai.apiMode = "auto" follows each model's default; "openai"/"anthropic" force the protocol.
            // In auto mode, protocol capability is detected dynamically from /v1/models
            // (supported_endpoint_types) at startup — no model IDs are hardcoded, so any
            // model that gains a capability is picked up automatically.
            // Priority in auto mode:
            //   1. model's own default apiMode (e.g. Claude models → anthropic)
            //   2. enableAnthropicApi (default false) + model supports anthropic → anthropic
            //   3. otherwise → openai
            const apiModeSetting = config.get<string>("bai.apiMode", "auto");
            const enableAnthropicApi = config.get<boolean>("bai.enableAnthropicApi", false);
            let apiMode: string;
            if (apiModeSetting === "openai" || apiModeSetting === "anthropic") {
                apiMode = apiModeSetting;
            } else if (um?.apiMode === "anthropic") {
                apiMode = "anthropic";
            ...
            const baseUrl = um?.baseUrl || "https://api.b.ai/v1/";
            ...
            const enableThirdPartyIndicator = config.get<boolean>("bai.enableThirdPartyTokenIndicator", true);
            ...
            const globalDelay = config.get<number>("bai.delay", 0);
            ...
            requestTimeoutMs = config.get<number>("bai.requestTimeout", 600000);
            ...
            // (401/403/429/502/503 — status codes and text patterns configurable).
            ...
            // failures (429/502/503 — retry later) from permanent ones (403/401 — check).
            ...
            if (!firstEntry) {
                ...
                throw new Error(l10n("Bai API key not found"));
            }
            ...
            while (true) {
                ...
                //    Balance ≤ minBalancePoints (default 0) → skip this key and try
            ...
            if (isTimeout || isForceTerminated) {
                ...
                throw new Error(l10n("Request timed out. The generation took too long. You can increase the timeout in settings (bai.requestTimeout)."));
            }
            ...
            console.error("[Bai] Chat request failed", {
                modelId: model.id,
                messageCount: messages.length,
            ...
            // Auto-hide the status bar after inactivity — it only reflects Bai
     ...
     * (openai / anthropic), streaming, and the ask_image vision
    ...
    private async _handleInterceptedToolCall(params: {
        ...
        const config = vscode.workspace.getConfiguration();
        const visionModelId = config.get<string>("bai.visionProxyModel", "kimi-k2.6");
        const maxRounds = config.get<number>("bai.visionMaxRounds", 5);
        ...
        for (let round = 1; round <= maxRounds; round++) {
            ...
            const roundTimeoutMs = vscode.workspace.getConfiguration().get<number>("bai.requestTimeout", 600000);
            ...
            if (params.apiMode === "anthropic") {
                ...
                if (params.um?.enable_thinking === true) {
                    // B.AI only supports enabled/disabled; adaptive → omit (model default).
                    if (params.um?.reasoning_effort !== 'adaptive') {
                        const maxTokens = body.max_tokens as number | undefined;
                        const budget = maxTokens ? Math.max(1024, Math.min(8192, Math.floor(maxTokens / 2))) : 8192;
                ...
                const normalizedUrl = params.baseUrl.replace(/\/+$/, "");
                    ...
                    if (params.um?.enable_thinking === true) {
                        // Bai OpenAI endpoint accepts only string thinking types
