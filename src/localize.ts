...
const zhCN: Record<string, string> = {
	...
	"Bai Provider API Key": "Bai 提供商 API 密钥",
	"Update your Bai API key": "更新您的 Bai API 密钥",
	"Enter your Bai API key": "输入您的 Bai API 密钥",
	"Bai API key cleared.": "Bai API 密钥已清除。",
	"Bai API key saved.": "Bai API 密钥已保存。",

	// provider.ts
	"Bai API key not found": "未找到 Bai API 密钥",
...
"Failed to generate commit message:": "生成提交消息失败：",
	...
	// Timeout error
	"Request timed out. The generation took too long. You can increase the timeout in settings (bai.requestTimeout).":
		"请求超时，生成内容过长。您可以在设置中增加超时时间（bai.requestTimeout）。",
	...
	"Enter the __Secure-authjs.session-token cookie (optional)": "输入 __Secure-authjs.session-token cookie（可选）",
	...
	"Edit the __Secure-authjs.session-token cookie (empty to clear)": "编辑 __Secure-authjs.session-token cookie（留空清除）",
	...
	"Enter the __Secure-authjs.session-token cookie for this key (optional)": "输入该 key 的 __Secure-authjs.session-token cookie（可选，可多个 key 共享同一 cookie）",
	...
	"Enter the __Secure-authjs.session-token cookie": "输入 __Secure-authjs.session-token cookie",
	"Enter the __Secure-authjs.session-token cookie value for this key": "输入该 key 对应的 __Secure-authjs.session-token cookie 值（留空清除）",
	...
	"Kimi K3 — vision-capable flagship": "Kimi K3 — 支持视觉的旗舰模型",
	"Qwen3.8 Max — text + image input, 1M context": "Qwen3.8 Max — 支持文本与图像输入，1M 上下文",
	"Gemini 3.1 Pro — vision-capable flagship": "Gemini 3.1 Pro — 支持视觉的旗舰模型",
	"Gemini 3.5 Flash — fast vision model": "Gemini 3.5 Flash — 快速视觉模型",
	"MiniMax M3 — vision-capable": "MiniMax M3 — 支持视觉",
	"Checking availability...": "正在检测可用性...",
	"Key is available": "检测通过：Key 可用",
	"Key balance is insufficient (≤ {0} points)": "积分不足（≤ {0} 积分），Key 标记为不可用",
