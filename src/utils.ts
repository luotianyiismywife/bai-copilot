...
import type { BaiModelItem, RetryConfig } from "./types";
...
export function normalizeUserModels(models: unknown): BaiModelItem[] {
    const list = Array.isArray(models) ? models : [];
    const out: BaiModelItem[] = [];
    for (const item of list) {
        if (!item || typeof item !== "object") {
            continue;
        }
        const provider = getModelProviderId(item);
        out.push({ ...(item as BaiModelItem), owned_by: provider });
    }
    return out;
}
...
export function createRetryConfig(): RetryConfig {
    const config = vscode.workspace.getConfiguration("bai.retry");
