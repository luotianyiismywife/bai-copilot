...
import { getApiModelIds, getAnthropicSupportedModelIds, isApiFetchSuccessful } from "./apiModelList";
...
// Key: model ID (API ID), Value: BaiModelItem
const _autoDiscoveredConfigs = new Map<string, BaiModelItem>();

/**
 * Module-level set of model IDs whose /v1/models entry includes "anthropic" in
 * supported_endpoint_types. Populated at startup (model list refresh) so
 * provider.ts can decide the API protocol dynamically without hardcoding
 * specific model IDs.
 */
let _anthropicModelIds = new Set<string>();

/**
 * Get the current set of Anthropic-protocol-capable model IDs
 * (supported_endpoint_types includes "anthropic"), as detected from /v1/models
 * at the last model list refresh.
...
/**
 * Build and store an BaiModelItem config for an auto-discovered model.
 * @param apiMode - Effective API protocol for the model ("openai" | "anthropic").
 */
 ...
 */
export function getAutoDiscoveredModelConfig(modelId: string): BaiModelItem | undefined {
 ...
 *
 * When the "bai.enableAutoModelDiscovery" setting is enabled (default),
...
export async function prepareLanguageModelChatInformation(
    ...
    const enableAutoDiscovery = config.get<boolean>("bai.enableAutoModelDiscovery", true);
    if (enableAutoDiscovery) {
        ...
        if (apiModelIds.size > 0 && isApiFetchSuccessful()) {
            ...
            // Step 0: Fetch Anthropic-protocol-capable model IDs
            ...
            if (newModelIds.length > 0) {
                ...
                for (const modelId of newModelIds) {
                    ...
                    if (newInfo) {
                        ...
                        // All B.AI models support the OpenAI-compatible endpoint.
                        storeAutoDiscoveredConfig(modelId, entry, "openai");
                        addedCount++;
                    }
                }
    ...
    // "anthropic"       → only models whose supported_endpoint_types includes "anthropic"
