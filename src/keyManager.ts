 ...
 *
 * 负责：
 * - SecretStorage 中多 key 的存取（`bai.apiKeys`），并自动迁移旧版单 key（`bai.apiKey`）
...
export interface ApiKeyEntry {
    ...
    /** 可选备注 */
    label?: string;
    /** __Secure-authjs.session-token cookie（可选；一个 cookie 可绑定多个 key，积分余额按 cookie 粒度查询） */
    cookie?: string;
...
const STORE_KEY = "bai.apiKeys";
const LEGACY_KEY = "bai.apiKey";
...
export function getRotationStatusCodes(): number[] {
    return getConfig().get<number[]>("apiKeyRotationStatusCodes", [401, 403, 429, 502, 503]);
}
...
/** 读取触发"瞬态整轮自动重试"的状态码列表（默认 [429, 502, 503]——限流/上游错误/服务端繁忙） */
export function getTransientRetryStatusCodes(): number[] {
    return getConfig().get<number[]>("transientRetryStatusCodes", [429, 502, 503]);
}
...
/**
 * 读取 API Key store。自动迁移旧版单 key（`bai.apiKey`）为单元素列表；
 ...
 * 匹配 `bai.transientRetryStatusCodes`（默认 [429, 503]）中的状态码。
 ...
 * - 403 / INSUFFICIENT_BALANCE / insufficient quota / "余额不足" → "balance"（B.AI 余额不足为 403）
 * - 401 → "invalid"
 * - 429 / RATE_LIMITED → "rate_limited"
 * - 502 / 503 → "server_error"
...
export function getKeyRotationReason(err: unknown): string {
    ...
    if (message.includes("[502]") || message.includes("status 502") || message.includes("[503]") || message.includes("status 503")) {
        return "server_error";
    }
    return "api_error";
}
