...
支持添加多个 Bai API Key（SecretStorage 加密存储），提供三种使用模式：
...
| `single` | 仅使用用户指定的"当前 key"；不可用时按 `bai.singleKeyFallback` 设置决定：`error`（默认，直接报错不切换）或 `switch`（自动切换到下一个可用 key，并右下角弹窗提示） |

**余额管理核心机制：**

- **主动预检（核心）**：每个 key 可绑定一个 `__Secure-authjs.session-token` cookie（**一个 cookie 可绑定多个 key**，共享余额）。请求前调用 `GET /api/usage-summary` 查询 cookie 余额详情，可用余额 ≤ `minBalanceCny`（设置可调，默认 0）时自动跳过该 key 并切换到下一个。
  ...
  "keys": [
    {
      ...
      "label": "工作号",               // 备注（可选）
      "cookie": "sess_xxxx...",        // __Secure-authjs.session-token cookie（可选，可多个 key 共享同一值）
...
| 函数 | 签名 | 职责 |
|------|------|------|
| `getApiKeyStore(secrets)` | `(secrets) => Promise<ApiKeyStore>` | 读取并缓存 store；自动迁移旧 `bai.apiKey`；JSON 损坏时回退修复 |
...
| 函数 | 签名 | 职责 |
|------|------|------|
| `queryBalanceDetail(cookie)` | `(cookie) => Promise<BalanceDetail>` | `GET https://chat.b.ai/api/usage-summary`，头 `Cookie: __Secure-authjs.session-token=<value>`，20s 超时；返回**完整余额详情** `{balanceCny, availableBalanceCny, expiringBalanceCny, nextExpiryAt}`（充值=available-expiring）；金额字段经 `toNumber` 强制转 number（API 可能返回字符串，非法值兜底 0）；401 抛"cookie 失效" |
...
| # | 情况 | 处理 | 覆盖 |
|---|------|------|------|
| I1 | 旧 `bai.apiKey` 存在、新格式不存在 | 迁移为单元素列表 `{keys:[{value}], activeIndex:0}` | ✅ |
...
```jsonc
"bai.apiKeyMode": { "type": "string", "enum": ["rotation", "single"], "default": "rotation" },
"bai.singleKeyFallback": {
...
"bai.apiKeyRotationStatusCodes": { "type": "array", "items": { "type": "number" }, "default": [401, 402, 429, 503] },
"bai.apiKeyRotationErrorPatterns": {
  "type": "array", "items": { "type": "string" },
  "default": ["余额不足", "insufficient balance", "INSUFFICIENT_BALANCE", "balance", "RATE_LIMITED", "UPSTREAM_RATE_LIMITED"]
},
"bai.transientRetryStatusCodes": { "type": "array", "items": { "type": "number" }, "default": [429, 503] },
"bai.transientRetryTimes": { "type": "number", "default": 3, "minimum": 0, "maximum": 10 },
"bai.apiKeyExhaustedCooldownMin": { "type": "number", "default": 10, "minimum": 0 },
"bai.balanceCheckEnabled": { "type": "boolean", "default": true },
"bai.minBalanceCny": { "type": "number", "default": 0, "minimum": 0 },
"bai.balanceCheckIntervalSec": { "type": "number", "default": 60, "minimum": 0 }
```
...
| 命令 | 用途 |
|------|------|
| `bai.manageApiKeys`（新增） | 打开 Key 管理 QuickPick（增删改查 + 检测可用性 + cookie 管理） |
| `bai.setApiKey`（保留） | 兼容旧版：设置单一 key（写入新格式单元素列表） |
| `bai.setModelPreset` 等 | 不变 |
